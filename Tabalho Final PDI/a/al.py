...
def combinar_grid_para_acc(grid_mask, grid_dir, grid_bri, paleta):
    """Achata as duas camadas (borda direcional + preenchimento por
    densidade) num único charmap de símbolos "ELF" (camada+caractere),
    resolvendo a ambiguidade de caracteres como '-' que existem com
    bitmaps diferentes em cada camada."""
    NL, NC = grid_mask.shape
    charmap = []
    for by in range(NL):
        row = []
        for bx in range(NC):
            if grid_mask[by, bx]:
                row.append('E' + grid_dir[by, bx])
            else:
...
def processar(entrada, nome_base, pasta_saida, bloco=8):
    ...
    for nome, paleta in [('10', PALETA_10), ('20', PALETA_20), ('40', PALETA_40)]:
        ...
        print(f"    → {saida}")
        charmap_combinado = combinar_grid_para_acc(grid_mask, grid_dir, grid_bri, paleta)
        acc_saida = os.path.join(pasta_saida, f'{nome_base}_{nome}chars_cores.acc')
        tamanho = save_acc(acc_saida, charmap_combinado)
        print(f"    → {acc_saida} ({tamanho} bytes)")
